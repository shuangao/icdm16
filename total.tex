...
In transfer learning, domain adaptation tries to exploit the knowledge from a source domain with a plentiful data to help learn a classifier for the target domain with a different distribution and little labeled training data. 
In this paper, we investigate this problem under the setting of \textit{Hypothesis Transfer Learning} (HTL) where we can only access the source model instead of the data. In real world application, this scenario is very common due to many reasons such as data credential.
As reported in \cite{ben2010theory}, negative transfer could happen when the source and target domain are not related, especially for the multi-class scenario. As we are not able to verify the source hypotheses in HTL, negative transfer could be an important issue in HTL.
...
To better estimate the transfer parameter, we propose a novel objective function to learn the better transfer parameter.
Experiment results show that our method can alleviate negative transfer and outperform other transfer methods in the different scenario.
...
\section{Transfer knowledge with data augmentation}\label{sec:prob}
\input{statement.tex}
